...
export function resolveContext(): vscode.WorkspaceFolder | undefined {
  ...
  // Tenta o workspace da aba ativa
...
/** Se o arquivo ativo é “marcador de projeto”, sobe diretórios e retorna a pasta do projeto. */
...
/** Varre agregadores (solução/workspace/grupo) no root ativo. */
export async function scanAggregators(ws: vscode.WorkspaceFolder): Promise<{ candidates: Candidate[] }> {
  ...
  // Prioriza .sln/.slnf, depois workspaces JS/TS, builds, Xcode/UE, etc.
  cands.sort((a, b) => scoreAggregator(b.label) - scoreAggregator(a.label));
  return { candidates: cands };
}

/** Varre projetos; usa heurística de “mais raso” como melhor palpite. */
